...
/**
 * Update the payment token prices in the contract
 * @param {Object} provider - Ethers.js provider
 * @param {Object} signer - Ethers.js signer with owner permissions
 * @param {boolean} updateAll - Whether to update all token prices or just XDC
 * @returns {Promise<Object>} - Updated prices
 */
export const updateContractPrices = async (
  provider,
  signer,
  updateAll = false
) => {
  try {
    ...
    // Tokens to update
    const tokensToUpdate = updateAll
      ? Object.entries(SUPPORTED_TOKENS)
      : Object.entries(SUPPORTED_TOKENS).filter(([symbol]) => symbol === "XDC");

    // Update prices for each token
    const updatePromises = tokensToUpdate.map(async ([symbol, details]) => {
      ...
      // Convert price to contract format (multiply by 10^18)
      const priceInWei = ethers.utils.parseUnits(prices[symbol].toString(), 18);

      // Send transaction to update price
      const tx = await presaleContract.setPaymentToken(
        details.address,
        priceInWei
      );
 ...
 */
export const checkPriceDiscrepancies = async (provider, threshold = 5) => {
  try {
    // Get current chain ID
    const chainId = await getCurrentChainId(provider);
